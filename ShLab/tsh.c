...
{
    int rc;
    if ((rc = sigemptyset(set)) < 0) unix_error("Sigemptyset error");
    return rc;
}
...
{
    int rc;
    if ((rc = sigaddset(set, signum)) < 0) unix_error("Sigaddset error");
    return rc;
}
...
{
    int rc;
    if ((rc = sigfillset(set)) < 0) unix_error("Sigfillset error");
    return rc;
}
...
{
    int rc;
    if ((rc = sigprocmask(how, set, oldset)) < 0) unix_error("Sigprocmask error");
    return rc;
}
...
{
    int rc;
    if ((rc = execve(filename, argv, envp)) < 0) unix_error("Execve error");
    return rc;
}
...
{
    int rc;
    if ((rc = dup(fd)) < 0) unix_error("Dup error");
    return rc;
}
...
void eval(char *cmdline)
{
    char *argv[MAXARGS];
    int bg;
    if ((bg = parseline(cmdline, argv)) == -1) return;
...
void sigint_handler(int sig)
{
    ...
    if ((pid = fgpid(jobs)) <= 0) unix_error("sigint_handler error");
    Kill(pid, sig);
    Sigprocmask(SIG_SETMASK, &prev_all, NULL);
...
void sigtstp_handler(int sig)
{
    ...
    if ((pid = fgpid(jobs)) <= 0) unix_error("sigtstp_handler error");
    Kill(pid, sig);
    Sigprocmask(SIG_SETMASK, &prev_all, NULL);
